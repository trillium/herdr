...
Configure static origins (non-tailnet hosts or overrides) with the `HERDR_FEDERATION_ORIGINS` environment variable. Set the socket directory for tailnet peers with `HERDR_FEDERATION_SOCKET_DIR`. Both are optional; Herdr auto-discovers Tailscale peers and uses the system temp directory by default.
...
| `HERDR_FEDERATION_SOCKET_DIR` | Directory where tailnet peers' forwarded API sockets are expected (when `experimental.federation` is enabled at startup). Discovered peers use the convention `<dir>/<origin-key>.sock` for their socket paths. Defaults to the system temp directory if unset. |
